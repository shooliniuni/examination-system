...
#include<iostream>
#include<conio.h>                       // for using  get.ch value
#include<cstdlib>                       // this libray use to shuff the quize
#include<windows.h>                    //usinf  for  system (cls)

using namespace std;

struct student{                       // struct student used to store data
                                     //struct is  group of data type for store data type
...
student st;                         // store data so we use data under function
...
void cppp(){
	...
	st.marks=0;                   // inisliazing marks

	char choice;                  // store user input and give output
	...
	st.random=rand()%6;          //generating random number for shuffl quize
	...
	switch(st.random)               // number store in this 
	...
	choice=getch();                                                  // user enter output

	if(choice=='D'||choice=='d'){                                   // this condition statement of use for answer. 

		cout<<choice<<" is correct Answer"<<endl<<endl;

		st.marks++;                                               // if marks is true then mark get increment
...
main(){
	...
	gets(st.nam);                             // here use gets because error not come after space which taken.

	cout<<"\t\tEnter rollno: ";

	gets(st.rollno);

	system("CLS");                           //  for next screen
	...
	select=getch();           // use to  store user input
